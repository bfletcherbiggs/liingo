defmodule Liingoew.Query.Category do
  use Liingoew.Query, model: Liingoew.Category
  def parent_ids do
    from cat in Liingoew.Category,
  ...
  def leaf_categories(parent_ids) when is_list(parent_ids), do: from cat in Liingoew.Category, where: not cat.id in ^parent_ids
  ...
  def names_and_id,
    do: from c in Liingoew.Category, select: {c.name, c.id}
