defmodule Liingoew.Web.CartController do
  ...
  def show(conn, _) do
    ...
    order_changeset = Liingoew.Order.cart_update_changeset(order, %{})
  ...
  def update(conn, %{"order" => order_params}) do
    order =
      conn.assigns.current_order
      |> Repo.preload([line_items: [variant: [:product, option_values: :option_type]]])
    order_changeset = Liingoew.Order.cart_update_changeset(order, order_params)
