defmodule Liingoew.Workflow.Checkout.Shipping do
  alias Ecto.Multi
  ...
  def changeset_for_step(order, params \\ %{}) do
    Liingoew.Order.shipping_changeset(order, params)
  end
  ...
  def view_data(repo, order) do
    available_shipping_methods =
      Liingoew.Query.ShippingMethod.enabled_shipping_methods(repo)
    order =
      order
      |> repo.preload(:shipment_units)
    %{proposed_shipping_methods: Liingoew.ShippingCalculator.calculate_applicable_shippings(order, available_shipping_methods)}
  end
  ...
  defp calculate_taxes(_changes, repo, order) do
    {:ok, Liingoew.TaxCalculator.calculate_taxes(repo, order)}
  end
end
