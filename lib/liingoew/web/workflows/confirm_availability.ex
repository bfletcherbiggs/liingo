defmodule Liingoew.Workflow.ConfirmAvailabilityInOrderChangeset do
  alias Ecto.Multi
  ...
  defp confirm_availability_of_line_items(_changes, repo, order_changeset) do
    ...
    {available, oos, discontinued, insufficient} = Liingoew.Order.check_line_items_for_availability(order)
    if available do
      {:ok, order_changeset}
    else
      changeset =
  ...
  defp add_error_message(changeset, :out_of_stock, oos) do
    Enum.reduce(oos, changeset, fn(line_item, changeset_acc) ->
      variant_display_name = Liingoew.Variant.display_name(line_item.variant)
