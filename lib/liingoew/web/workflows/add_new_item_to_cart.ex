defmodule Liingoew.Workflow.AddNewItemToCart do
  alias Ecto.Multi
  ...
  def steps(repo, variant, cart, quantity) do
    ...
    |> Multi.append(Liingoew.Workflow.CheckVariantAvailability.steps(variant, quantity, changeset))
  ...
  defp ensure_product_has_no_variants_if_master(_changes, repo, variant, changeset) do
    ...
    if variant.is_master and Liingoew.Query.Product.has_variants_excluding_master?(repo, product) do
  ...
  defp line_item_insert_changeset(variant, cart, quantity) do
    ...
    |> Liingoew.LineItem.changeset(%{order_id: cart.id, unit_price: variant.cost_price, add_quantity: quantity})
  end
